...
%\newcommand{\metropolis@disablesectionpage}{\AtBeginSection{% intentionally empty1
%}
%}
\AtBeginSection[]
                  {
...
\title{Construire un corpus annoté pour une langue peu dotée : \\  annotation collaborative d'un corpus de l'alsacien en partie du discours}
  ...
  \Wider[3em]{
    \begin{columns}
      \begin{column}{0.4\textwidth}
        \begin{tikzpicture}
          \node (img0) {\includegraphics[width=1.2\textwidth]{figures/France-uni.png}};
        \end{tikzpicture}
      \end{column}
...
\begin{frame}{Le continuum dialectal}
  \begin{columns}
    ...
    \begin{column}{0.85\textwidth}
      ...
      \small
      \textit{Mer müess mache dass d'Kerisch mittess im Dorf bliebt. \\~\\ %(Alémanique du nord)
        Mer müess màche  dàss d'Kìrisch mìtel vum Dorf blibt.\\~\\ % Alémanique du nord (graphie orthal)
        Mer müass màcha dàss d’Kîrch mittess îm Dorf blibt.\\~\\ % (Alémanique du Sud (graphie GRIMAL))
        M'r müess màcha dàss d'Kìrch mìtel vum Dorf blibt.\\~\\  % (Orthal - Sud)
      }
...
\begin{frame}{L'alsacien : une langue \og peu dotée \fg{} pour le TAL}
  ...
  \begin{itemize}
\item un tokéniseur\footnotemark[1]
    \visible{\item German \tool{TreeTagger}~\cite{Schmid1997}\footnotemark[2]}
    \visible<3->{\smash{\raisebox{3\dimexpr0.3\baselineskip-9.7\itemsep+5.5\parskip}{$\left.\rule{0pt}{2\dimexpr1\baselineskip+2\itemsep-1\parskip}\right\}$\ \parbox{2.8cm}{\center utilisés comme \\ \textbf{outils de pré-annotation}}}}} 
  \visible<2->{\item \tool{MElt}~\cite{Denis2010}, \\ entraîné régulièrement avec nos \\ données annotées (\tool{MElt$_{ALS}$})}\\
  \end{itemize}
  \bigskip
  \visible<3->{
    \Wider[4em]{
  ...
  %%   \visible<3->{\includegraphics[width=0.8\linewidth]{figures/niveaux-trans.png}}
  %% \end{column}
  \footnotetext[1]{Fourni par Delphine Bernhard, LiLPa, Strasbourg.}
...
\centering
  ...
  \end{figure}
    \hfill\cite{Fort2012} \\
    \centering
  l'annotation manuelle \og classique \fg{} est un processus \textbf{long} et \textbf{coûteux} \\
  \scriptsize par exemple, la construction du \tool{Prague Treebank} a été évaluée à 600~000 \$ par~\cite{Bohmova2001}
}
\end{frame}
...
\subsection{Choix méthodologiques et ressources initiales}
%% POS
\begin{frame}{Jeu d'étiquettes utilisé}
  \begin{table}[!ht]
    ...
    \resizebox{0.7\linewidth}{!}{
      \begin{tabular}{l|l|l}
        \toprule
        \textbf{Classes ouvertes} & \textbf{Classes fermées} & \textbf{Autres} \\
        \hline
        \textsc{adj} & \textsc{adp} & \textsc{punct} \\
        \textsc{adv} & \textsc{aux} & \textsc{sym} \\
        \textsc{intj} & \textsc{conj} & \textsc{x} \\
        \textsc{noun} & \textsc{det} &  \\
        \textsc{propn} & \textsc{num} & 	 \\
        \textsc{verb} & \textsc{part} &  \\
        &\textsc{pron} &\\
        &\textsc{sconj} & \\            \toprule  
      \end{tabular}
    }
  \end{table}
  \centering
  Liste des étiquettes utilisées selon le classement des créateurs du jeu d'étiquettes.\footnotemark[1]
   
  \footnotetext[1]{http://universaldependencies.org/u/pos/all.html}
\end{frame}
...
\begin{frame}{Conditions initiales : les ressources - lexiques}
  \Wider[2.5em]{
    \begin{itemize}
    \item     un lexique de 322 mots fréquents ~\cite{bernhard_es_2013} ($L_{MO}$)
...
\begin{frame}{Conditions initiales : les ressources - corpus} 
  ...
  \item un corpus de référence (102 phrases) annoté par deux expertes linguistes et utilisé pour évaluer notre système\\
    ...
    \visible<2->{
    \item un corpus brut de 26~000 tokens issus de la Wikipédia alémanique\footnotemark[2] 
    }
  \end{itemize}
  \begin{columns}
    \begin{column}{0.2\textwidth}
      \begin{tikzpicture}
        \node (img0) {\includegraphics[height=4.5cm]{figures/Elsass2.png}};
      \end{tikzpicture}
    \end{column}
    \begin{column}{0.8\textwidth}
      ...
      \begin{table}[]
        ...
        \begin{tabular}{l|l|c|c}
          ...
          \multicolumn{1}{l|}{} & \sla Pièce de théâtre
          & 231 & \pause \\ \hline \hline 
          \multicolumn{1}{l|}{Corpus brut} & \nla \tool{Wikipédia}  & \multicolumn{2}{p{3cm}}{8~833} \\ \toprule  
        \end{tabular}
        %}
      \end{table}
    \end{column}
  \end{columns}
  \footnotetext[2]{http///als.wikipedia.org}
\end{frame}
...
\begin{frame}{\tool{Bisame}}
  \Wider[4em]{
    \begin{tikzpicture}
      ...
      \pause
      \node (img1) at (img0) {\includegraphics[width=1\textwidth]{figures/frontpage-fr.png}};
      \pause
      \node (img2) at (img0) {\includegraphics[width=1\textwidth]{figures/frontpage-fr-2.png}};
    \end{tikzpicture}  
  }   
\end{frame}
...
\begin{frame}{Annoter \textit{via} \tool{Bisame}}
  ...
  \begin{center}
    Si \tool{German Treetagger} \textcolor{red}{!=} \tool{MElt$_{ALS}$}: \\
      \begin{figure}     
        \includegraphics[width=0.95\textwidth]{figures/annot2.png}
    \end{figure}
    \visible<2->{Si \tool{German Treetagger} \textcolor{greensolution}{\textbf{=}} \tool{MElt$_{ALS}$}: \\
      \begin{figure}   
    ...
    \begin{center} 
	  ...
	  axis x line=bottom,
	    ...
	    scaled ticks=false,
          ...
          \addplot[black,fill=black!40,bar shift=-.075cm,line width=0.05pt] table [
            y=Nb1
	    ...
	    legend style={at={(1,1.4)}}
          ...
          \addplot[fill=black!70,bar shift=0cm,line width=0.05pt]table [
            y=Nb2
          ] {data/participants-and-annotations-per-week.dat};
...
\begin{frame}{Les performances de \tool{MElt$_{ALS}$}}
    ...
    %\Wider[3em]{
    \centering
    pour 10 phrases d'environ 10 mots, 2 mots par phrase sont mal annotés \\
  \scriptsize (\tool{MElt$_{FR}$} présente 97,7~\% d'exactitude \cite{Denis2010})  \\~\\

  \visible<2>{les performances du \tool{Stanford tagger}  sur l'alsacien sont presque atteintes mais c'est encore très insuffisant}
...
\begin{frame}{Conclusion et perspectives}
  ...
  \begin{itemize}
  \item  \small Manque de variété des corpus disponibles \visible<2->{\textbf{\normalsize \color{greensolution} $\rightarrow$  permettre la soumission de \textbf{textes personnels}~\cite{Liberm2016}. }}
   \visible<3->{\item  \small Qualité des annotations imparfaite} \visible<4->{\textbf{ \normalsize \color{greensolution}$\rightarrow$  repenser le guide d'annotation et la stratégie d'évaluation des participants.} }
   \visible<5->{\item  \small Ludification insuffisante pour maintenir \textbf{motivation} \visible<6->{et \textbf{volition}}} \visible<7->{\textbf{ \normalsize \color{greensolution} $\rightarrow$~rendre la plate-forme autonome (e-mails de rappel, \og une phrase par jour \fg{}).}}
  \end{itemize}
  }
  \begin{center}
  \visible<5>{\og capacité à \textbf{attirer} des participants \fg{}}\\
  \visible<6>{\og capacité à \textbf{faire revenir} des participants~\cite{Fenouillet2009} \fg{}}
  \end{center}
\end{frame}
