  ...
  \begin{columns}
    ...
    \begin{column}{0.85\textwidth}
      ...
      \textit{Mer müess mache ass d'Kerisch mittess im Dorf bliebt . \\~\\
        Mer müass màcha àss d’Kîrech mittess îm Dorf blibt.\\~\\ % (Parler de Guebwiller Haut Rhin)
        M'r müass màcha àss d'Kirch Mitt'l im Dorf blibt.\\~\\  % (Orthal)
      }
...
\begin{frame}{L'alsacien : une langue \og peu dotée \fg{} pour le TAL}
  \centering
  \small \textbf{\uline{1}} outil pour le traitement automatique de l'alsacien : \\~\\~\\
  \begin{columns}
    \visible<2->{
    \begin{column}{0.46\textwidth}
      \centering
      un tokéniseur  \footnotemark[1] \\~\\
      \begin{table}[]
        \centering
...
\begin{frame}{Conditions initiales : les ressources - lexiques}
  \Wider[3em]{
    \begin{itemize}
    \item     un lexique de 322 \og{mots-outils}\fg{}~\cite{bernhard_es_2013} ($L_{MO}$)
...
\begin{frame}{Annoter \textit{via} \tool{Bisame}}
  ...
  \begin{center}
    \visible<2->{Si \tool{German Treetagger} \textcolor{red}{!=} \tool{MElt$_{ALS}$}: \\
      \begin{figure}     
        \includegraphics[width=0.95\textwidth]{figures/annot2.png}
    \end{figure}}
    \visible<3->{Si \tool{German Treetagger} \textcolor{greensolution}{\textbf{=}} \tool{MElt$_{ALS}$}: \\
      \begin{figure}   
...
\begin{frame}{Traitement des annotations concurrentes - 1}
  ...
  \end{columns}
  \centering
  \visible<3->{\small chaque participant $P$ est doté d'un score de confiance \\ (réévalué régulièrement) \\~\\   $Confiance_{P}=\frac{NbAnn_{Ref,Correcte}}{NbAnn_{Ref}}$} \\ \bigskip
...
%    \resizebox{1\linewidth}{!}{
  ...
  \end{table}
  \centering
  \visible<2>{$C_{\textbf{\textit{Stroßburger}}}=$\textsc{adj}}
\end{frame}
...
\begin{frame}{Un nouveau corpus annoté de l'alsacien}
  %\Wider[3em]{
    ...
    \item \textbf{16~628 annotations}, dont :
      \begin{itemize}
        \item 7~795 sur le corpus de référence (pour l'évaluation)
        \item 8~833 sur le corpus brut (pour la production d'annotation)
      \end{itemize}
      \end{itemize}
  %}
    \visible<2->{$\rightarrow$ un nouveau corpus annoté de l'alsacien de \textbf{5~725 tokens}}\\
...
\begin{frame}{Les performances de \tool{MElt$_{ALS}$}}
  ...
  \small
  \visible<2>{les performances de l'existant sur l'alsacien sont presque atteintes mais c'est encore très insuffisant}
  }
\end{frame}
...
\begin{frame}{Conclusion et perspectives}
  ...
  \begin{itemize}
  \item Manque de variété des corpus disponibles \visible<2->{\textbf{ \small \color{greensolution} $\rightarrow$  permettre la soumission de \textbf{textes personnels}~\cite{Liberm2016}. }}
   \visible<3->{\item Qualité des annotations imparfaite} \visible<4->{\textbf{ \small \color{greensolution}$\rightarrow$  repenser le guide d'annotation et la stratégie d'évaluation des participants.} }
   \visible<5->{\item Ludification insuffisante pour maintenir motivation et volition~\cite{Fenouillet2009}} \visible<6->{\textbf{ \small \color{greensolution} $\rightarrow$  rendre la plate-forme autonome (e-mails de rappel, \og une phrase par jour \fg{}).}}
  \end{itemize}
  }
...
\begin{frame}{Conclusion et perspectives}
    ...
    \item améliorer le jeu d'étiquettes (à compléter)
    \item<2-> travailler à une meilleure prise en compte de la diversité dialectale et orthographique.
    \item<3-> adapter la plate-forme à une autre langue peu dotée : le créole guadeloupéen (\small projet en cours avec Gwladys Feler, étudiante en M1 Master LPL : Langue et informatique à Université Paris Sorbonne)
